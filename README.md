[Prodessional ML Certificate Coursera.pdf](https://github.com/user-attachments/files/24421466/Prodessional.ML.Certificate.Coursera.pdf)
<img width="1050" height="820" alt="image" src="https://github.com/user-attachments/assets/c49f8ca5-ae06-488b-ad77-2da553d2b2f3" /> 




# 🎓 Machine Learning Specialization – Andrew Ng (DeepLearning.AI)

This repository contains my hands-on implementations, course labs, notes, and assignments from the Professional Machine Learning Specialization by Andrew Ng (DeepLearning.AI).

The specialization is divided into 3 industry-aligned modules, covering core ML theory, practical implementation, and real-world use cases.

---

## 📜 Certificates Included

✔️ Supervised Machine Learning  
✔️ Advanced Machine Learning  
✔️ Unsupervised Learning, Recommenders & Reinforcement Learning

---
